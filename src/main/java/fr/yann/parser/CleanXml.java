...
public class CleanXml {
	...
	// 1_2_3_4__5_6_7_8_9
	private static LigneBilan parseStringWithJoker(String line, EpreuveEnum epreuveEnum, SexeEnum sexeEnum, LastRang lastRang, int annee) {
		...
		if (m.find()) {
			...
			if (prenom.contains("(")) {
				...
				nomPrenom.prenom = prenom.substring(0, indexParenthese - 1);
			}
	...
	public static LigneBilan traiteLineAtypique(String line, EpreuveEnum epreuveEnum, SexeEnum sexeEnum, LastRang lastRang, int annee) {
		...
		return parseStringWithJoker(lineWithoutCF, epreuveEnum, sexeEnum, lastRang, annee);
	}
